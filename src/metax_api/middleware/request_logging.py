    ...
    def get_username(self, request):
        ...
        if "Basic" in auth_header:
            try:
                user_type = "service"
                user = (
                    b64decode(auth_header.split(" ")[1]).decode("utf-8").split(":")[0]
                )
            except:
            ...
            try:
                user_type = "end_user"
                user = json_loads(
                    b64decode(auth_header.split(" ")[1].split(".")[1] + "===").decode(
                        "utf-8"
                    )
                )["CSCUserName"]
            except:
                ...
                # token later too.
                _logger.info(
                    "Faulty token: Could not extract username from bearer token"
                )
        else:
            _logger.info(
                "HTTP Auth method not basic or bearer - unable to get username"
            )
