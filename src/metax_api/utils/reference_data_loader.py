...
class ReferenceDataLoader:
    ...
    def populate_cache_reference_data(cls, cache, settings=django_settings):
        ...
        if "reference_data" not in reference_data_check.keys():
            _logger.warning(
                "Key reference_data missing from reference data - "
        ...
        if "organization_data" not in reference_data_check.keys():
            _logger.warning(
                "Key organization_data missing from reference data - "
                "something went wrong during cache population?"
            )
            errors = True
        ...
        if not errors:
            ...
            cache.set(
                "ref_data_up_to_date",
                True,
                ex=django_settings.REFERENCE_DATA_RELOAD_INTERVAL,
            )

        _logger.info(
            "ReferenceDataLoader - %s"
            % ("failed to populate cache" if errors else "cache populated")
        )
            ...
            for type_name in [b["key"] for b in aggr_types["aggregations"]["types"]["buckets"]]:
                ...
                for row in all_rows:
                    ...
                    try:
                        ...
                        _logger.warning(
                            "Elasticsearch document missing uri in index {0} type {1}: {2}".format(
                    ...
                    try:
                        ...
                        entry["code"] = None
                        _logger.warning(
                            "Elasticsearch document missing code in index {0} type {1}: {2}".format(
                                index_name, type_name, row
                            )
                        )
                    ...
                    if (
                        type_name == "license"
                        and "same_as" in row["_source"]
                        and len(row["_source"]["same_as"]) > 0
                    ):
