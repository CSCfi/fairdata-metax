...
class FileApiReadBasicTests(FileApiReadCommon):
    ...
    def test_expand_relations(self):
        ...
        self.assertEqual(
            "date_created" in response.data["parent_directory"],
            True,
            response.data["parent_directory"],
        )
...
class FileApiReadGetRelatedDatasets(FileApiReadCommon):
    ...
    def test_get_related_datasets_ok_2(self):
        ...
        """
        file_identifiers = File.objects.filter(id__in=[1, 2, 3, 4, 5]).values_list(
    ...
    def test_keysonly(self):
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=files&keysonly", [1, 2, 121], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=files&keysonly=false", [1, 2], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(type(response.data), dict, response.data)  # Return by keys

        response = self.client.post(
            "/rest/files/datasets?keys=datasets&keysonly", [1, 2, 14], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_get_detailed_related_datasets_ok_2(self):
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=files", file_identifiers, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 5)
    ...
    def test_get_detailed_related_files_ok_1(self):
        ...
        """
        response = self.client.post("/rest/files/datasets?keys=datasets", [1], format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 1)
    ...
    def test_get_detailed_related_files_ok_2(self):
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=datasets", dataset_identifiers, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 5)
    ...
    def test_get_right_files_and_datasets(self):
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=datasets", [cr.data["identifier"]], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=files", [testfile["identifier"]], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        for keys, values in response.data.items():
            ...
            self.assertEqual(
                "cr955e904-e3dd-4d7e-99f1-3fed446f96d1"
                and "cr955e904-e3dd-4d7e-99f1-3fed446f9612"
                and "cr955e904-e3dd-4d7e-99f1-3fed446f9611" in values,
                True,
                response.data,
            )
        ...
        self.assertEqual(
            sorted(response.data["cr955e904-e3dd-4d7e-99f1-3fed446f9611"]),
            sorted(identifiers),
            response.data,
        )

        response = self.client.post(
            "/rest/files/datasets?keys=files", ["pid:urn:20"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        # Dataset 11 should be found from results
        self.assertTrue(
            "cr955e904-e3dd-4d7e-99f1-3fed446f9611" in response.data["pid:urn:20"],
            response.data,
        )
    ...
    def test_get_related_datasets_files_not_found(self):
        ...
        response = self.client.post(
            "/rest/files/datasets?keys=files", ["doesnotexist"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 0)
        ...
        # Support for ?detailed
        response = self.client.post(
            "/rest/files/datasets?detailed", ["doesnotexist"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 0)
...
class FileApiReadEndUserAccess(FileApiReadCommon):
    ...
    def test_user_can_read_owned_files(self):
        ...
        self.assertEqual(
            len(response.data),
            0,
            "should return 200 OK, but user projects has no files",
        )
