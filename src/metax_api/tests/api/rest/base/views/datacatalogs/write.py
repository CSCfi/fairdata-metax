...
class DataCatalogApiWriteBasicTests(DataCatalogApiWriteCommon):
    def test_identifier_is_auto_generated(self):
        ...
        self.assertNotEqual(
            response.data["catalog_json"].get("identifier", None),
            None,
    ...
    def test_create_identifier_already_exists(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            "already exists" in response.data["catalog_json"]["identifier"][0],
            True,
            response.data,
        )
    ...
    def test_delete(self):
        ...
        self.assertEqual(
            dc_deleted.date_modified,
            dc_deleted.date_removed,
...
class DataCatalogApiWriteReferenceDataTests(DataCatalogApiWriteCommon):
    ...
    def test_create_data_catalog_populate_fields_from_reference_data(self):
        ...
        dc["access_rights"]["has_rights_related_agent"][0]["identifier"] = refs["organization"][
            "code"
        ]
    ...
    def _assert_uri_copied_to_identifier(self, refs, new_dc):
        self.assertEqual(
            refs["field_of_science"]["uri"], new_dc["field_of_science"][0]["identifier"]
        )
        self.assertEqual(refs["language"]["uri"], new_dc["language"][0]["identifier"])
        self.assertEqual(
            refs["access_type"]["uri"],
            new_dc["access_rights"]["access_type"][0]["identifier"],
        )
        self.assertEqual(
            refs["license"]["uri"], new_dc["access_rights"]["license"][0]["identifier"]
        )
    ...
    def _assert_label_copied_to_pref_label(self, refs, new_dc):
        self.assertEqual(
            ...
            new_dc["field_of_science"][0].get("pref_label", None),
        )
        self.assertEqual(
    ...
    def _assert_label_copied_to_title(self, refs, new_dc):
        self.assertEqual(
            refs["license"]["label"],
            new_dc["access_rights"]["license"][0].get("title", None),
        )
