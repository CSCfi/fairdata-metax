...
class DirectoryApiReadFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_get_files_by_path(self):
        ...
        response = self.client.get(
            "/rest/directories/files?path=%s&project=%s"
    ...
    def test_read_directory_get_files_by_path_not_found(self):
        response = self.client.get(
            "/rest/directories/files?path=%s&project=%s" % ("doesnotexist", "doesnotexist")
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_read_directory_recursively_with_dirs_only_and_max_depth(self):
        ...
        """
        response = self.client.get(
            "/rest/directories/2/files?recursive=true&directories_only=true&depth=3"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual("directories" in response.data, True)
...
class DirectoryApiReadFileBrowsingRetrieveSpecificFieldsTests(DirectoryApiReadCommon):
    def test_retrieve_requested_directory_fields_only(self):
        response = self.client.get(
            "/rest/directories/3/files?directory_fields=identifier,directory_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_directory_byte_size_and_file_count(self):
        ...
        """
        response = self.client.get(
            "/rest/directories/3/files?directory_fields=identifier,byte_size"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"][0].keys()), 2)
        self.assertEqual("identifier" in response.data["directories"][0], True)
        ...
        response = self.client.get(
            "/rest/directories/3/files?directory_fields=identifier,file_count"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"][0].keys()), 2)
        self.assertEqual("identifier" in response.data["directories"][0], True)
        ...
        response = self.client.get(
            "/rest/directories/3/files?directory_fields=identifier,file_count&cr_identifier=3"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"][0].keys()), 2)
    ...
    def test_retrieve_requested_file_and_directory_fields_only(self):
        response = self.client.get(
            "/rest/directories/3/files?file_fields=identifier&directory_fields=id"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_not_retrieving_not_allowed_directory_fields(self):
        ...
        response = self.client.get(
            "/rest/directories/3/files?file_fields=parent,id&directory_fields=;;drop db;,id"
        )
        ...
        response = self.client.get(
            "/rest/directories/3/files?file_fields=parent&directory_fields=or"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class DirectoryApiReadCatalogRecordFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_for_catalog_record(self):
        ...
        """
        response = self.client.get(
            "/rest/directories/3/files?cr_identifier=%s"
    ...
    def test_read_directory_for_catalog_record_recursively(self):
        ...
        """
        response = self.client.get(
            "/rest/directories/1/files?recursive&cr_identifier=%s&depth=*"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            "/rest/directories/1/files?recursive&cr_identifier=1&depth=*&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
    ...
    def test_read_directory_for_not_catalog_record_recursively(self):
        ...
        file_list = list(File.objects.filter(record__pk=1).values_list("id", flat=True))
        response = self.client.get(
            "/rest/directories/1/files?recursive&depth=*&not_cr_identifier=%s"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        # directory itself is not found. the error contains details about the 400
        response = self.client.get(
            "/rest/directories/1/files?recursive&not_cr_identifier=notexisting"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_directory_byte_size_and_file_count(self):
        ...
        cr_data["research_dataset"]["directories"] = [
            {
                ...
                "title": "test dir",
                "use_category": {
                    "identifier": cr_with_dirs.research_dataset["directories"][0]["use_category"][
                        "identifier"
                    ]
                },
            }
        ...
        response = self.client.get(
            "/rest/directories/%d/files?cr_identifier=%s&include_parent" % (dr.id, cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_directory_byte_size_and_file_count_in_parent_directories(self):
        ...
        def _assertDirectoryData(id, parent_data):
            response = self.client.get(
                "/rest/directories/%d/files?cr_identifier=%d&include_parent" % (id, cr_id)
            )
        ...
        def _assertDirectoryData_not_cr_id(id, parent_data):
            ...
            response = self.client.get(
                "/rest/directories/%d/files?not_cr_identifier=%d&include_parent" % (id, cr_id)
            )
        ...
        dirs = set(
            [
                Directory.objects.get(identifier=dr["identifier"]).id
...
class DirectoryApiReadCatalogRecordFileBrowsingAuthorizationTests(DirectoryApiReadCommon):
    ...
    def test_returns_ok_for_open_catalog_record_if_no_authorization(self):
        ...
        # open catalog record
        # Because user has no projects, request with not_cr_identifier returns 404.
        self._assert_ok(open_cr_json, "no", not_cr_status=status.HTTP_404_NOT_FOUND)
    ...
    def test_returns_ok_for_login_catalog_record_if_no_authorization(self):
        ...
        # Because user has no projects, request with not_cr_identifier returns 404.
        self._assert_ok(login_cr_json, "no", not_cr_status=status.HTTP_404_NOT_FOUND)
    ...
    def test_returns_ok_for_open_catalog_record_if_owner_authorization(self):
        ...
        # Because user has no access to project, request with not_cr_identifier returns 403.
        self._assert_ok(open_cr_json, "owner", not_cr_status=status.HTTP_403_FORBIDDEN)
    ...
    def test_returns_ok_for_login_catalog_record_if_owner_authorization(self):
        ...
        # Because user has no access to project, request with not_cr_identifier returns 403.
        self._assert_ok(login_cr_json, "owner", not_cr_status=status.HTTP_403_FORBIDDEN)
    ...
    def test_returns_ok_for_restricted_catalog_record_if_owner_authorization(self):
        ...
        # Because user has no access to project, request with not_cr_identifier returns 403.
        self._assert_ok(restricted_cr_json, "owner", not_cr_status=status.HTTP_403_FORBIDDEN)
    ...
    def test_returns_ok_for_embargoed_catalog_record_if_available_reached_and_no_authorization(
        ...
        # Because user has no projects, request with not_cr_identifier returns 404.
        self._assert_ok(available_embargoed_cr_json, "no", not_cr_status=status.HTTP_404_NOT_FOUND)
    ...
    def _assert_forbidden(self, cr_json, credentials_type):
        ...
        response = self.client.get(
            "/rest/directories/{0}/files?cr_identifier={1}".format(dir_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        response = self.client.get(
            "/rest/directories/{0}/files?not_cr_identifier={1}".format(dir_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def _assert_ok(self, cr_json, credentials_type, not_cr_status=status.HTTP_200_OK):
        dir_file_amt = cr_json["research_dataset"]["directories"][0]["details"]["file_count"]
        dir_id = cr_json["research_dataset"]["directories"][0]["identifier"]
        cr_id = cr_json["identifier"]
        self._set_http_authorization(credentials_type)
        response = self.client.get(
            "/rest/directories/{0}/files?cr_identifier={1}&recursive&depth=*".format(dir_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data), dir_file_amt)

        response = self.client.get(
            "/rest/directories/{0}/files?not_cr_identifier={1}&recursive&depth=*".format(
                dir_id, cr_id
            )
        )

        self.assertEqual(response.status_code, not_cr_status, response.data)
        if not_cr_status == status.HTTP_200_OK:
            self.assertEqual(len(response.data), 0)
...
class DirectoryApiReadCatalogRecordFileBrowsingRetrieveSpecificFieldsTests(DirectoryApiReadCommon):
    ...
    def test_retrieve_requested_directory_fields_only(self):
        response = self.client.get(
            "/rest/datasets/12?file_details&directory_fields=identifier,directory_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
    ...
    def test_retrieve_directory_byte_size_and_file_count(self):
        ...
        """
        response = self.client.get(
            "/rest/datasets/12?file_details&directory_fields=identifier,byte_size"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["research_dataset"]["directories"][0]["details"].keys()),
        ...
        response = self.client.get(
            "/rest/datasets/12?file_details&directory_fields=identifier,file_count"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["research_dataset"]["directories"][0]["details"].keys()),
    ...
    def test_retrieve_requested_file_fields_only(self):
        response = self.client.get(
            "/rest/datasets/12?file_details&file_fields=identifier,file_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_requested_file_and_directory_fields_only(self):
        response = self.client.get(
            "/rest/datasets/12?file_details&file_fields=identifier&directory_fields=id"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DirectoryApiReadEndUserAccess(DirectoryApiReadCommon):
    ...
    def test_browsing_in_cr_context(self):
        ...
        self._use_http_authorization(method="bearer", token=self.token)
        response = self.client.get(
            "/rest/directories/3/files?cr_identifier={0}".format(cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_browsing_in_not_cr_context(self):
        """
        Browsing files with not_cr_identifier should be forbidden if user does not have access to the project.
        """
        cr = CatalogRecord.objects.get(pk=1)
        self._use_http_authorization(method="bearer", token=self.token)
        response = self.client.get(
            "/rest/directories/3/files?not_cr_identifier={0}".format(cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)

        self._update_token_with_project_of_directory(3)
        response = self.client.get(
            "/rest/directories/3/files?not_cr_identifier={0}".format(cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
...
class DirectoryApiReadPaginationTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_with_dirs_only_and_pagination(self):
        ...
        """
        file_dict = self._get_dirs_files_ids("/rest/directories/24/files?directories_only")[
            "directories"
        ]
...
class DirectoryApiReadFileNameDirectoryNameTests(DirectoryApiReadCommon):
    ...
    def test_browsing_directory_with_directory_and_file_name(self):
        ...
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir_1&file_name=file_name_120"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(len(response.data["files"]), 1)
        ...
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir&file_name=not_existing"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 5)
        self.assertEqual(len(response.data["files"]), 0)
    ...
    def test_browsing_directory_with_file_and_dir_name_and_pagination(self):
        ...
        response = self.client.get(
            "/rest/directories/24/files?file_name=0&pagination&limit=10&offset=10"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]["directories"]), 0)
        self.assertEqual(len(response.data["results"]["files"]), 10)
        ...
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir_&pagination&limit=3"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # first page with limit of 3 should return first filtered directories
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir_1&file_name=0&pagination"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_dirs_only(self):
        ...
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir_5&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(response.data.get("files"), None)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_recursive(self):
        ...
        response = self.client.get(
            "/rest/directories/24/files?directory_name=dir_5&file_name=5&recursive&depth=*"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_cr_identifier_and_not_cr_identifier(
        ...
        response = self.client.get(
            "/rest/directories/17/files?directory_name=phase&cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.get(
            "/rest/directories/17/files?directory_name=phase&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 1)

        response = self.client.get(
            "/rest/directories/17/files?directory_name=2&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 1)
        ...
        response = self.client.get(
            "/rest/directories/17/files?file_name=&cr_identifier=13&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(response.data.get("files"), None)
        ...
        response = self.client.get(
            "/rest/directories/16/files?file_name=name_2&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 1)

        response = self.client.get(
            "/rest/directories/16/files?file_name=name_1&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 0)