...
class DirectoryApiWriteTests(DirectoryApiWriteCommon):
    def test_create_files_for_catalog_record(self):
        ...
        file1_title = response.data["file_name"]
        ...
        file2_title = response.data["file_name"]
        ...
        file3_title = response.data["file_name"]
        ...
        cr["research_dataset"]["preferred_identifier"] = cr_rd_identifier
        response = self.client.put("/rest/datasets/{}".format(cr_id), cr, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)

        # getting dataset files from /
        response = self.client.get(
            "/rest/directories/{}/files?cr_identifier={}&fields".format(root_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.get(
            "/rest/directories/{}/files?cr_identifier={}&include_parent&{}".format(
        ...
        response = self.client.get(
            "/rest/directories/{}/files?not_cr_identifier={}&include_parent&{}".format(
                dirs[0]["id"], cr_id, fields
            )
        )
        self.assertEqual(
        ...
        response = self.client.patch("/rest/datasets/{}".format(cr_id), cr, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        # getting dataset files from /
        response = self.client.get(
            "/rest/directories/{}/files?not_cr_identifier={}&fields".format(root_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND, "Directory must be empty")
