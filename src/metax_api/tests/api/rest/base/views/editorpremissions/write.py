...
class EditorUserPermissionApiWriteBasicTests(EditorUserPermissionApiWriteCommon):
    ...
    def test_write_editor_permission_remove_users(self):
        ...
        response = self.client.post("/rest/datasets/%d/editor_permissions/users" % self.crid, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
        response = self.client.delete("/rest/datasets/%d/editor_permissions/users/%s" % (self.crid, data.get('user_id')))
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        for user in response.data:
            if user.get('role') == 'creator':
                response = self.client.delete(
                    "/rest/datasets/%d/editor_permissions/users/%s" % (self.crid, user.get('user_id')))
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, response.data)
            else:
                response = self.client.delete(
                    "/rest/datasets/%d/editor_permissions/users/%s" % (self.crid, user.get('user_id')))
    ...
    def test_write_editor_permission_add_removed_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        removed_user = EditorUserPermission.objects_unfiltered.get(user_id="new_editor",
                                                                   editor_permissions_id=self.permissionid)
