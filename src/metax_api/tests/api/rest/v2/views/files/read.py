...
class FileApiReadBasicTests(FileApiReadCommon):
    ...
    def test_read_file_list_filter_by_project_and_path(self):
        ...
        response = self.client.get(
            "/rest/v2/files?project_identifier=%s&file_path=%s" % (proj, path)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], file_count)
...
class FileApiReadGetRelatedDatasets(FileApiReadCommon):
    ...
    def test_keysonly(self):
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=files&keysonly", [1, 2, 121], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=files&keysonly=false", [1, 2], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(type(response.data), dict, response.data)  # Return by keys

        response = self.client.post(
            "/rest/v2/files/datasets?keys=datasets&keysonly", [1, 2, 14], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_get_detailed_related_datasets_ok_2(self):
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=files", file_identifiers, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 5)
    ...
    def test_get_detailed_related_files_ok_2(self):
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=datasets", dataset_identifiers, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 5)
    ...
    def test_get_right_files_and_datasets(self):
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=datasets",
            [cr.data["identifier"]],
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=files", ["pid:urn:20"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_get_related_datasets_files_not_found(self):
        ...
        response = self.client.post(
            "/rest/v2/files/datasets?keys=files", ["doesnotexist"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 0)
        ...
        # Support for ?detailed
        response = self.client.post(
            "/rest/v2/files/datasets?detailed", ["doesnotexist"], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self._assert_results_length(response, 0)
