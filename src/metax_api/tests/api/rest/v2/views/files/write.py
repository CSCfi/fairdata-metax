...
class FileApiWriteCommon(APITestCase, TestClassUtils):
    ...
    def _check_project_root_byte_size_and_file_count(self, project_identifier):
        ...
        """
        byte_size = File.objects.filter(project_identifier=project_identifier).aggregate(
            Sum("byte_size")
        )["byte_size__sum"]
        file_count = File.objects.filter(project_identifier=project_identifier).count()

        response = self.client.get("/rest/v2/directories/root?project=%s" % project_identifier)
...
class FileApiWriteReferenceDataValidationTests(FileApiWriteCommon):
    ...
    def test_file_format_version_with_invalid_file_format_when_format_version_given_1(
        self,
    ):
        ...
        self.assertEqual(
            "file_characteristics.file_format" in response.data["file_characteristics"],
            True,
        )
    ...
    def test_file_format_version_with_invalid_format_version_when_file_format_has_versions_1(
        self,
    ):
        ...
        self.assertEqual("file_characteristics" in response.data.keys(), True)
        self.assertEqual(
            "file_characteristics.format_version" in response.data["file_characteristics"],
            True,
        )
    ...
    def test_file_format_version_with_empty_format_version_when_file_format_has_no_version_1(
        ...
        self.test_new_data["file_characteristics"]["file_format"] = self.ff_without_version[
            "input_file_format"
        ]
        response = self.client.post("/rest/v2/files", self.test_new_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_file_format_version_with_empty_format_version_when_file_format_has_no_version_2(
        self,
    ):
        self.test_new_data["file_characteristics"]["file_format"] = self.ff_without_version[
            "input_file_format"
    ...
    def test_file_format_version_with_valid_file_format_and_valid_file_version_1(self):
        ...
        self.test_new_data["file_characteristics"]["format_version"] = self.ff_with_version[
    ...
    def test_file_characteristics_is_validated_on_update(self):
        ...
        response = self.client.put(
            "/rest/v2/files/%s" % response.data["identifier"],
            response.data,
            format="json",
        )
...
class FileApiWriteCreateTests(FileApiWriteCommon):
    ...
    def test_create_file(self):
        ...
        newly_created_file_name = (
            "   MX  .201015_Suomessa_tavattavat_ruokasammakot_ovat_väritykseltään_vaihtelevia_"
    ...
    def test_allow_creating_previously_deleted_file(self):
        ...
        """
        response = self.client.post("/rest/v2/files", self.test_new_data, format="json")
        response = self.client.delete("/rest/v2/files/%d" % response.data["id"], format="json")
    ...
    def test_create_file_error_json_validation(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            "file_characteristics" in response.data.keys(),
            True,
            "The error should concern the field file_characteristics",
        )
        self.assertEqual(
            "metadata_modified" in response.data["file_characteristics"][0],
            True,
            "The error should contain the name of the erroneous field",
        )
        self.assertEqual(
            "Json path:" in response.data["file_characteristics"][0],
            True,
    ...
    def test_create_file_not_allowed_checksum_algorithm(self):
        ...
        for algo in ["sha2", "sha256", "sha-256"]:
            ...
            # somethind to do with the fact that POST requests to /rest/v2/files do not normally
    ...
    def test_create_file_list(self):
        ...
        response = self.client.post(
            "/rest/v2/files",
            [self.test_new_data, self.second_test_new_data],
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
        ...
        self._check_project_root_byte_size_and_file_count(
            response.data["success"][0]["object"]["project_identifier"]
        )

        # ensure structure of some specific fields is the same as when single files are created
        self.assertEqual(
            "identifier" in response.data["success"][0]["object"]["file_storage"], True
        )
        self.assertEqual(
            "identifier" in response.data["success"][0]["object"]["parent_directory"],
            True,
        )
    ...
    def test_create_file_list_error_one_fails(self):
        ...
        self.assertEqual(
            "identifier" in response.data["failed"][0]["errors"],
            True,
    ...
    def test_parameter_ignore_already_exists_errors(self):
        ...
        response = self.client.post(
            "/rest/v2/files?ignore_already_exists_errors",
            [self.test_new_data, self.second_test_new_data],
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
...
class FileApiWriteCreateDirectoriesTests(FileApiWriteCommon):
    ...
    def test_create_file_hierarchy_from_single_file(self):
        ...
        file_path = (
            "/project_y_FROZEN/Experiment_1/path/of/lonely/file_and_this_also_has_to_support"
        ...
        response = self.client.post("/rest/v2/files", f, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
    ...
    def test_create_file_append_to_existing_directory(self):
        ...
        file_count_before = (
            Directory.objects.filter(
                ...
                directory_path="/project_x_FROZEN/Experiment_X/Phase_1",
            )
            .first()
            .files.all()
            .count()
    ...
    def test_create_file_hierarchy_from_file_list_with_no_existing_files(self):
        ...
        response = self.client.post("/rest/v2/files", experiment_1_file_list, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
        self.assertEqual("success" in response.data.keys(), True)
    ...
    def test_create_file_hierarchy_from_file_list_with_existing_files(self):
        ...
        for i, f in enumerate(experiment_2_file_list):
            f["file_path"] = f["file_path"].replace(
                "/project_y_FROZEN/Experiment_1",
                "/project_y_FROZEN/Experiment_2/Phase_1/Data",
            )
            f["identifier"] = "%s-%d" % (f["file_path"], i)

        response = self.client.post("/rest/v2/files", experiment_2_file_list, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
        self.assertEqual("success" in response.data.keys(), True)
        self.assertEqual(len(response.data["success"]), 12)
        self.assertEqual(len(response.data["failed"]), 0)
    ...
    def test_filepath_starts_with_slash(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue(
            "file path should start with '/' to point to the root" in response.data["file_path"][0]
        )
        ...
        for dir_path, ids in dirs_dict.items():
            if dir_path == "/":
                self.assertEqual(
                    ids["parent_dir_id"],
                    None,
                    "root dir '/' should not have a parent directory",
                )
                continue
            expected_parent_dir_path = dirname(dir_path)
            self.assertEqual(
                ids["parent_dir_id"],
                dirs_dict[expected_parent_dir_path]["dir_id"],
    ...
    def _assert_file_parent_dirs(self, dirs_dict, response):
        ...
        for entry in response.data["success"]:
            ...
            self.assertEqual(
                f["parent_directory"]["id"],
                dirs_dict[excpected_parent_dir_path]["dir_id"],
                "parent dir not as expected.",
            )
    ...
    def _form_complex_list_from_test_file(self):
        ...
        for i, d in enumerate(dir_data):
            ...
            files[-1].update(
                d, identifier="pid:urn:test:file:%d" % i, project_identifier="project_y"
            )
...
class FileApiWriteUpdateTests(FileApiWriteCommon):
    ...
    def test_update_file(self):
        f = self.client.get("/rest/v2/files/1").data
        f["file_format"] = "csv"
        response = self.client.put("/rest/v2/files/%s" % f["identifier"], f, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_file_error_required_fields(self):
        ...
        response = self.client.put(
            "/rest/v2/files/%s" % self.identifier, self.test_new_data, format="json"
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            "project_identifier" in response.data.keys(),
            True,
            "Error for field 'project_identifier' is missing from response.data",
        )

    def test_update_file_not_found(self):
        response = self.client.put("/rest/v2/files/doesnotexist", self.test_new_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_update_file_allowed_projects_ok(self):
        f = self.client.get("/rest/v2/files/1").data
        response = self.client.put(
            "/rest/v2/files/%s?allowed_projects=%s" % (f["identifier"], f["project_identifier"]),
            f,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_file_allowed_projects_fail(self):
        f = self.client.get("/rest/v2/files/1").data
        response = self.client.put(
            "/rest/v2/files/%s?allowed_projects=nopermission" % f["identifier"],
            f,
            format="json",
        )
    ...
    def test_file_update_list(self):
        f1 = self.client.get("/rest/v2/files/1").data
        f2 = self.client.get("/rest/v2/files/2").data
        ...
        response = self.client.put("/rest/v2/files", [f1, f2], format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_file_update_list_error_one_fails(self):
        ...
        self.assertEqual(
            "file_frozen" in response.data["failed"][0]["errors"],
            True,
    ...
    def test_file_update_list_allowed_projects_ok(self):
        ...
        response = self.client.put(
            "/rest/v2/files?allowed_projects=project_x,y,z", [f1, f2], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_file_update_list_allowed_projects_fail(self):
        ...
        f1 = self.client.get("/rest/v2/files/1").data
        f2 = self.client.get("/rest/v2/files/39").data
    ...
    def test_file_update_list_allowed_projects_empty_value(self):
        f1 = self.client.get("/rest/v2/files/1").data
        response = self.client.put("/rest/v2/files?allowed_projects=", [f1], format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
    ...
    def test_file_update_list_allowed_projects_not_list(self):
        ...
        res = self.client.patch(
            "/rest/v2/files?allowed_projects=y,z,project_x", new_data_1, format="json"
        )
...
class FileApiWritePartialUpdateTests(FileApiWriteCommon):
    ...
    def test_update_file_partial(self):
        ...
        response = self.client.patch("/rest/v2/files/%s" % self.identifier, new_data, format="json")

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual("file_name" in response.data.keys(), True)
        self.assertEqual(
            "file_path" in response.data.keys(),
            True,
        ...
        self.assertEqual(
            ...
            "new_file_name",
            "Field file_name was not updated",
        )
    ...
    def test_update_partial_allowed_projects_ok(self):
        ...
        response = self.client.patch(
            "/rest/v2/files/%s?allowed_projects=%s" % (self.identifier, self.pidentifier),
            new_data,
            format="json",
        )
    ...
    def test_update_partial_allowed_projects_fail(self):
        ...
        response = self.client.patch(
            "/rest/v2/files/%s?allowed_projects=noproject" % self.identifier,
            new_data,
            format="json",
        )
    ...
    def test_file_partial_update_list(self):
        ...
        response = self.client.patch("/rest/v2/files", [test_data, second_test_data], format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            2,
            "response.data should contain 2 changed objects",
        )
        self.assertEqual(
            "file_characteristics" in response.data["success"][0]["object"],
            True,
        ...
        updated_file = File.objects.get(pk=1)
        self.assertEqual(
            updated_file.project_identifier,
            new_project_identifier,
            "project_identifier did not update",
        )
    ...
    def test_file_partial_update_list_allowed_projects_ok(self):
        ...
        res = self.client.patch(
            "/rest/v2/files?allowed_projects=y,z,project_x",
...
class FileApiWriteDeleteTests(FileApiWriteCommon):
    ...
    def test_delete_single_file_ok_destroy_leading_dirs(self):
        ...
        response = self.client.post("/rest/v2/files", test_data, format="json")
        self.assertEqual(
            Directory.objects.filter(project_identifier=project_identifier).exists(),
            True,
        )

        response = self.client.delete("/rest/v2/files/%s" % response.data["id"])
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual("deleted_files_count" in response.data, True, response.data)
        self.assertEqual(response.data["deleted_files_count"], 1, response.data)
    ...
    def test_delete_single_file_404(self):
        response = self.client.delete("/rest/v2/files/doesnotexist")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_bulk_delete_files_identifiers_not_found(self):
        ...
        response = self.client.delete("/rest/v2/files", identifiers, format="json")
    ...
    def test_bulk_delete_files_some_identifiers_not_found(self):
        ...
        self._check_project_root_byte_size_and_file_count(
            File.objects_unfiltered.get(pk=1).project_identifier
        )
    ...
    def test_bulk_delete_files_in_single_directory_1(self):
        ...
        response = self.client.delete("/rest/v2/files", file_ids, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_bulk_delete_files_from_root(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(
            response.data.get("deleted_files_count", None),
        ...
        self.assertEqual(
            Directory.objects_unfiltered.filter(project_identifier="project_x").count(),
            0,
    ...
    def _assert_files_available_and_removed(self, project_identifier, available, removed):
        ...
        """
        self.assertEqual(
            File.objects.filter(project_identifier=project_identifier).count(),
            available,
            "files should not be retrievable from removed=False scope",
        )
        self.assertEqual(
            File.objects_unfiltered.filter(
                project_identifier=project_identifier, removed=True
            ).count(),
            removed,
...
class FileApiWriteRestoreTests(FileApiWriteCommon):
    def test_restore_files_ok(self):
        ...
        """
        response = self.client.delete("/rest/v2/files/1")
        response = self.client.delete("/rest/v2/files/2")
        response = self.client.delete("/rest/v2/files/3")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        deleted_files = File.objects_unfiltered.filter(pk__in=[1, 2, 3]).values(
            "identifier", "parent_directory_id"
        )

        response = self.client.post(
            "/rest/v2/files/restore",
            [f["identifier"] for f in deleted_files],
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_restore_files_recreate_missing_directories(self):
        ...
        response = self.client.get(
            "/rest/files?project_identifier=%s&fields=identifier&pagination=false" % proj,
            format="json",
        )
        file_identifiers = [f["identifier"] for f in response.data]

        self.client.delete("/rest/v2/files", file_identifiers, format="json")

        deleted_directory_ids = File.objects_unfiltered.filter(
            identifier__in=file_identifiers
        ...
        self.assertEqual("restored_files_count" in response.data, True, response.data)
        self.assertEqual(
            response.data["restored_files_count"], len(file_identifiers), response.data
        )
    ...
    def test_check_files_belong_to_one_project(self):
        ...
        f2 = (
            File.objects_unfiltered.filter()
            .exclude(project_identifier=f1.project_identifier)
            .first()
        )
        response = self.client.delete("/rest/v2/files/%d" % f1.id)
        response = self.client.delete("/rest/v2/files/%d" % f2.id)
        response = self.client.post(
            "/rest/v2/files/restore", [f1.identifier, f2.identifier], format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class FileApiWriteXmlTests(FileApiWriteCommon):
    ...
    def test_xml_api(self):
        ...
        data = (
            '<?xml version="1.0" encoding="utf-8"?><root><stuffia>tauhketta yeah</stuffia></root>'
        )

        # create
        response = self.client.post(
            "/rest/v2/files/1/xml?namespace=breh", data, content_type=content_type
        )
        self.assertEqual(response.status_code in (200, 201, 204), True)
        self.assertEqual("<?xml" in response.data, True)

        response = self.client.post(
            "/rest/v2/files/1/xml?namespace=bruh", data, content_type=content_type
        )
        self.assertEqual(response.status_code in (200, 201, 204), True)
        self.assertEqual("<?xml" in response.data, True)

        # get list
        response = self.client.get("/rest/v2/files/1/xml", content_type=content_type)
        self.assertEqual(response.status_code in (200, 201, 204), True)
        self.assertEqual(isinstance(response.data, list), True, response.data)
        self.assertEqual("breh" in response.data, True)

        # get single
        response = self.client.get("/rest/v2/files/1/xml?namespace=breh", content_type=content_type)
        self.assertEqual(response.status_code in (200, 201, 204), True)
        self.assertEqual("<?xml" in response.data, True)
        ...
        # get updated again
        response = self.client.get(
            "/rest/v2/files/1/xml?namespace=breh",
...
class FileApiWriteEndUserAccess(FileApiWriteCommon):
    ...
    def test_user_can_update_files_in_their_projects(self):
        ...
        response = self.client.get("/rest/v2/files?project_identifier=%s" % proj, format="json")
        ...
        response = self.client.put("/rest/v2/files/%s" % file["id"], file, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.put("/rest/v2/files", [file], format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class FileApiWriteDryrunTest(FileApiWriteCommon):
    ...
    """
    Test query param ?dryrun=bool separately for post /rest/v2/files api, due to special
    behavior in POST /rest/v2/files.
    ...
    def test_dryrun(self):
        ...
        """
        response = self.client.post(
            "/rest/v2/files?what&dryrun=true&other", self.test_new_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
