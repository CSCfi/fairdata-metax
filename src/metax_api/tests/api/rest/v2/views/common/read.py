...
class ApiReadGetDeletedObjects(CatalogRecordApiReadCommon):
    ...
    def test_removed_parameter_gets_correct_amount_of_objects(self):
        ...
        self.assertEqual(
            len(results),
            0,
            "Without removed objects remove=true should return 0 results",
        )
        ...
        self.assertEqual(
            len(results),
            initial_amt - amt_to_delete,
            "Non-removed object amount is incorrect",
        )
    ...
    def test_read_catalog_record_list_pagination_2(self):
        for param in ["pagination=true", "pagination", ""]:
            ...
            self.assertEqual(
                response.data["results"][0]["id"],
                3,
                "Id of first result should have been 3",
            )
        ...
        for order in ("preservation_state", "-preservation_state"):
            ...
            for offset in range(0, 20, 5):
                ...
                response = self.client.get(
                    f"/rest/v2/datasets?limit={limit}&offset={offset}&ordering={order}"
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
                ...
                from_db = [
                    r
                    for r in CatalogRecord.objects.filter()
                    .order_by(order)
...
class ApiReadQueryParamTests(CatalogRecordApiReadCommon):
    ...
    def test_checksum_field_for_file(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue(
            "is not part of" in response.data["detail"][0],
            "Should complain about field not found",
        )
