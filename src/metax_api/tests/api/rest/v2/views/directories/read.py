...
class DirectoryApiReadCommon(APITestCase, TestClassUtils):
    ...
    def _get_dirs_files_ids(self, url):
        ...
        if isinstance(file_data, dict):
            return {
                key: [f["id"] for f in file_data[key]]
...
class DirectoryApiReadBasicTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_details_by_pk(self):
        response = self.client.get("/rest/v2/directories/%s" % self.pk)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            hasattr(response, "data"),
            True,
            "Request response object is missing attribute 'data'",
        )
    ...
    def test_read_directory_details_by_identifier(self):
        response = self.client.get("/rest/v2/directories/%s" % self.identifier)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            hasattr(response, "data"),
            True,
    ...
    def test_read_directory_details_not_found(self):
        response = self.client.get("/rest/v2/directories/shouldnotexist")
...
class DirectoryApiReadFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_get_files(self):
        ...
        Test browsing files
        """
        response = self.client.get("/rest/v2/directories/2/files")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get("/rest/v2/directories/5/files")
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(len(response.data["files"]), 0)

        response = self.client.get("/rest/v2/directories/6/files")
    ...
    def test_read_directory_get_files_recursively(self):
        ...
        response = self.client.get("/rest/v2/directories/1/files?recursive=true&depth=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 20)

        # dir id 3 contains 5 files, but recursively 20
        response = self.client.get("/rest/v2/directories/3/files?recursive=true&depth=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 20)

        # dir id 4 contains 5 files, but recursively 15
        response = self.client.get("/rest/v2/directories/4/files?recursive=true&depth=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 15)

        # dir id 5 contains 0 files
        response = self.client.get("/rest/v2/directories/5/files?recursive=true&depth=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 10)

        # dir id 6 contains 10 files
        response = self.client.get("/rest/v2/directories/6/files?recursive=true&depth=*")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 10)

    def test_read_directory_get_files_file_not_found(self):
        response = self.client.get("/rest/v2/directories/not_found/files")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_read_directory_get_project_root_directory(self):
        response = self.client.get("/rest/v2/directories/root?project=project_x")
    ...
    def test_read_directory_get_project_root_directory_not_found(self):
        response = self.client.get("/rest/v2/directories/root?project=project_xyz")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_read_directory_get_project_root_directory_parameter_missing(self):
        response = self.client.get("/rest/v2/directories/root")
    ...
    def test_read_directory_get_files_by_path(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/files?path=%s&project=%s"
    ...
    def test_read_directory_get_files_by_path_not_found(self):
        response = self.client.get(
            "/rest/v2/directories/files?path=%s&project=%s" % ("doesnotexist", "doesnotexist")
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_read_directory_get_files_by_path_check_parameters(self):
        response = self.client.get("/rest/v2/directories/files")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.get("/rest/v2/directories/files?path=something")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.get("/rest/v2/directories/files?project=something")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_read_directory_recursively_with_max_depth(self):
        ...
        """
        response = self.client.get("/rest/v2/directories/2/files?recursive=true&depth=3")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 10)
    ...
    def test_read_directory_recursively_with_dirs_only_and_max_depth(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/2/files?recursive=true&directories_only=true&depth=3"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual("directories" in response.data, True)
    ...
    def test_read_directory_recursively_with_no_depth(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/3/files?recursive=true&directories_only=true"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual("directories" in response.data, True)
        self.assertEqual("directories" in response.data["directories"][0], True)
    ...
    def test_read_directory_return_directories_only(self):
        response = self.client.get("/rest/v2/directories/3/files?directories_only")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual("files" in response.data, False)

    def test_read_directory_with_include_parent(self):
        response = self.client.get("/rest/v2/directories/3/files?include_parent")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(len(response.data["files"]), 5)
    ...
    def test_read_directory_files_sorted_by_file_path(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["files"][0]["file_path"],
            "/project_x_FROZEN/Experiment_X/file_name_1",
        )
        self.assertEqual(
            response.data["files"][1]["file_path"],
            "/project_x_FROZEN/Experiment_X/file_name_2",
        )
        self.assertEqual(
            response.data["files"][2]["file_path"],
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["results"]["files"][0]["file_path"],
            "/project_x_FROZEN/Experiment_X/file_name_2",
        )
        self.assertEqual(
            response.data["results"]["files"][1]["file_path"],
            "/project_x_FROZEN/Experiment_X/file_name_3",
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data[0]["file_path"], "/project_x_FROZEN/Experiment_X/file_name_1"
        )
        self.assertEqual(
            response.data[1]["file_path"], "/project_x_FROZEN/Experiment_X/file_name_2"
        )
        self.assertEqual(
            response.data[2]["file_path"], "/project_x_FROZEN/Experiment_X/file_name_3"
        )

        response = self.client.get("/rest/v2/directories/3/files?recursive&cr_identifier=2")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data[0]["file_path"], "/project_x_FROZEN/Experiment_X/file_name_3"
        )
        self.assertEqual(
            response.data[1]["file_path"], "/project_x_FROZEN/Experiment_X/file_name_4"
        )
    ...
    def test_read_directory_directories_sorted_by_directory_path(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["directories"][0]["directory_path"], "/prj_112_root/other")
        self.assertEqual(
            response.data["directories"][1]["directory_path"],
            "/prj_112_root/random_folder",
            response.data,
        )
        self.assertEqual(
            response.data["directories"][2]["directory_path"],
            "/prj_112_root/science_data_A",
        )
        self.assertEqual(
            response.data["directories"][3]["directory_path"],
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["results"]["directories"][0]["directory_path"],
            "/prj_112_root/science_data_A",
        )
        self.assertEqual(
            response.data["results"]["directories"][1]["directory_path"],
            "/prj_112_root/science_data_B",
        )
...
class DirectoryApiReadFileBrowsingRetrieveSpecificFieldsTests(DirectoryApiReadCommon):
    def test_retrieve_requested_directory_fields_only(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/3/files?directory_fields=identifier,directory_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_directory_byte_size_and_file_count(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/3/files?directory_fields=identifier,byte_size"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"][0].keys()), 2)
        self.assertEqual("identifier" in response.data["directories"][0], True)
        ...
        response = self.client.get(
            "/rest/v2/directories/3/files?directory_fields=identifier,file_count"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"][0].keys()), 2)
        self.assertEqual("identifier" in response.data["directories"][0], True)
    ...
    def test_retrieve_requested_file_fields_only(self):
        response = self.client.get("/rest/v2/directories/3/files?file_fields=identifier,file_path")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_requested_file_and_directory_fields_only(self):
        response = self.client.get(
            "/rest/v2/directories/3/files?file_fields=identifier&directory_fields=id"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_not_retrieving_not_allowed_directory_fields(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/3/files?file_fields=parent,id&directory_fields=;;drop db;,id"
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            any(field in response.data["files"][0].keys() for field in allowed_file_fields)
        )
        self.assertTrue(
            any(field in response.data["directories"][0].keys() for field in allowed_dir_fields)
        )

        response = self.client.get(
            "/rest/v2/directories/3/files?file_fields=parent&directory_fields=or"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class DirectoryApiReadCatalogRecordFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_catalog_record_and_not_catalog_record_not_ok(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/3/files?cr_identifier=1&not_cr_identifier=2"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue(
            "one query parameter of 'cr_identifier' and 'not_cr_identifier'"
            in response.data["detail"][0]
        )
    ...
    def test_read_directory_for_catalog_record(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/3/files?cr_identifier=%s"
    ...
    def test_read_directory_for_catalog_record_directory_does_not_exist(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/4/files?cr_identifier=%s"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        ...
        response = self.client.get(
            "/rest/v2/directories/4/files?not_cr_identifier=%s"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
    ...
    def test_read_directory_for_catalog_record_recursively(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/directories/1/files?recursive&cr_identifier=%s&depth=*"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            "/rest/v2/directories/1/files?recursive&cr_identifier=1&depth=*&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        ...
        # directory itself is not found. the error contains details about the 400
        response = self.client.get(
            "/rest/v2/directories/1/files?recursive&cr_identifier=notexisting"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_read_directory_for_not_catalog_record_recursively(self):
        ...
        file_list = list(File.objects.filter(record__pk=1).values_list("id", flat=True))
        response = self.client.get(
            "/rest/v2/directories/1/files?recursive&depth=*&not_cr_identifier=%s"
            % CatalogRecord.objects.get(pk=1).identifier
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        # directory itself is not found. the error contains details about the 400
        response = self.client.get(
            "/rest/v2/directories/1/files?recursive&not_cr_identifier=notexisting"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class DirectoryApiReadCatalogRecordFileBrowsingAuthorizationTests(DirectoryApiReadCommon):
    ...
    def test_returns_ok_for_open_catalog_record_if_service_authorization(self):
        ...
        # Verify /rest/v2/directories/<dir_id>/files?cr_identifier=cr_id returns dir files with service
    ...
    def test_returns_ok_for_open_catalog_record_if_owner_authorization(self):
        ...
        # Verify /rest/v2/directories/<dir_id>/files?cr_identifier=cr_id returns dir files with owner authorization for
    ...
    def test_returns_ok_for_restricted_catalog_record_if_service_authorization(self):
        ...
        # Verify /rest/v2/directories/<dir_id>/files?cr_identifier=cr_id returns dir files with service
        # authorization for restricted catalog record
    ...
    def test_returns_ok_for_embargoed_catalog_record_if_available_reached_and_no_authorization(
        ...
        # Verify /rest/v2/directories/<dir_id>/files?cr_identifier=cr_id returns dir files without authorization
    ...
    def test_returns_forbidden_for_restricted_catalog_record_if_no_authorization(self):
        ...
        # Verify /rest/v2/directories/<dir_id>/files?cr_identifier=cr_id returns forbidden without authorization
    ...
    def _assert_forbidden(self, cr_json, credentials_type):
        ...
        response = self.client.get(
            "/rest/v2/directories/{0}/files?cr_identifier={1}".format(dir_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        response = self.client.get(
            "/rest/v2/directories/{0}/files?not_cr_identifier={1}".format(dir_id, cr_id)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def _assert_ok(self, cr_json, credentials_type):
        ...
        self._set_http_authorization(credentials_type)
        response = self.client.get(
            "/rest/v2/directories/{0}/files?cr_identifier={1}&recursive&depth=*".format(
                dir_id, cr_id
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.get(
            "/rest/v2/directories/{0}/files?not_cr_identifier={1}&recursive&depth=*".format(
                dir_id, cr_id
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
...
class DirectoryApiReadCatalogRecordFileBrowsingRetrieveSpecificFieldsTests(DirectoryApiReadCommon):
    ...
    def test_retrieve_requested_directory_fields_only(self):
        response = self.client.get(
            "/rest/v2/datasets/12?include_user_metadata&file_details&directory_fields=identifier,directory_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["research_dataset"]["directories"][0]["details"].keys()),
            2,
        )
        self.assertEqual(
            "identifier" in response.data["research_dataset"]["directories"][0]["details"],
            True,
        )
        self.assertEqual(
            "directory_path" in response.data["research_dataset"]["directories"][0]["details"],
            True,
        )
    ...
    def test_retrieve_directory_byte_size_and_file_count(self):
        ...
        """
        response = self.client.get(
            "/rest/v2/datasets/12?include_user_metadata&file_details&directory_fields=identifier,byte_size"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["research_dataset"]["directories"][0]["details"].keys()),
        ...
        self.assertEqual(
            "byte_size" in response.data["research_dataset"]["directories"][0]["details"],
            True,
        )

        response = self.client.get(
            "/rest/v2/datasets/12?include_user_metadata&file_details&directory_fields=identifier,file_count"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["research_dataset"]["directories"][0]["details"].keys()),
        ...
        self.assertEqual(
            "file_count" in response.data["research_dataset"]["directories"][0]["details"],
            True,
        )

    def test_retrieve_requested_file_fields_only(self):
        response = self.client.get(
            "/rest/v2/datasets/12?include_user_metadata&file_details&file_fields=identifier,file_path"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["research_dataset"]["files"][0]["details"].keys()), 2)
        self.assertEqual(
            "identifier" in response.data["research_dataset"]["files"][0]["details"],
            True,
        )
        self.assertEqual(
            "file_path" in response.data["research_dataset"]["files"][0]["details"],
            True,
        )

    def test_retrieve_requested_file_and_directory_fields_only(self):
        response = self.client.get(
            "/rest/v2/datasets/12?include_user_metadata&file_details&file_fields=identifier&directory_fields=id"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DirectoryApiReadEndUserAccess(DirectoryApiReadCommon):
    ...
    """
    Test End User Access permissions when browsing files using /rest/v2/directories api.
    ...
    def test_user_can_browse_files_from_their_projects(self):
        """
        Ensure users can only read files from /rest/v2/directories owned by them.
        """
        self._use_http_authorization(method="bearer", token=self.token)
    ...
    def test_browsing_in_cr_context(self):
        ...
        self._use_http_authorization(method="bearer", token=self.token)
        response = self.client.get(
            "/rest/v2/directories/3/files?cr_identifier={0}".format(cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_browsing_in_not_cr_context(self):
        ...
        self._use_http_authorization(method="bearer", token=self.token)
        response = self.client.get(
            "/rest/v2/directories/3/files?not_cr_identifier={0}".format(cr.identifier)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
...
class DirectoryApiReadPaginationTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_with_default_limit_pagination(self):
        ...
        self.assertEqual(
            ...
            file_dict["directories"][0],
        )
        self.assertEqual(
            response.data["results"]["directories"][9]["id"],
            file_dict["directories"][9],
        )
        ...
        self.assertEqual(
            response.data["results"]["directories"][0]["id"],
            file_dict["directories"][10],
        )
        self.assertEqual(
            response.data["results"]["directories"][3]["id"],
            file_dict["directories"][13],
        )
    ...
    def test_read_directory_with_custom_limit_pagination(self):
        ...
        self.assertEqual(
            response.data["results"]["directories"][0]["id"],
            file_dict["directories"][12],
        )
        self.assertEqual(
            response.data["results"]["directories"][1]["id"],
            file_dict["directories"][13],
        )
        ...
        self.assertEqual(
            response.data["results"]["directories"][0]["id"],
            file_dict["directories"][8],
        )
        self.assertEqual(
            response.data["results"]["directories"][3]["id"],
            file_dict["directories"][11],
        )
    ...
    def test_read_directory_with_dirs_only_and_pagination(self):
        ...
        """
        file_dict = self._get_dirs_files_ids("/rest/v2/directories/24/files?directories_only")[
            "directories"
        ]
...
class DirectoryApiReadFileNameDirectoryNameTests(DirectoryApiReadCommon):
    ...
    def test_browsing_directory_with_file_name(self):
        ...
        response = self.client.get("/rest/v2/directories/24/files?file_name=")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["files"]), 51)

        response = self.client.get("/rest/v2/directories/24/files?file_name=_name_1")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["files"]), 21)

        response = self.client.get("/rest/v2/directories/24/files?file_name=0")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["files"]), 15)

        response = self.client.get("/rest/v2/directories/24/files?file_name=_name_118")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["files"]), 1)

    def test_browsing_directory_with_directory_name(self):

        response = self.client.get("/rest/v2/directories/24/files?directory_name=")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 5)

        response = self.client.get("/rest/v2/directories/24/files?directory_name=dir_1")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)

        response = self.client.get("/rest/v2/directories/24/files?directory_name=dir")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 5)

    def test_browsing_directory_with_directory_and_file_name(self):

        response = self.client.get("/rest/v2/directories/24/files?directory_name=&file_name=")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 5)
        self.assertEqual(len(response.data["files"]), 51)

        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir_1&file_name=file_name_120"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(len(response.data["files"]), 1)
        ...
        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir&file_name=not_existing"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 5)
        self.assertEqual(len(response.data["files"]), 0)
    ...
    def test_browsing_directory_with_file_and_dir_name_and_pagination(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/24/files?file_name=0&pagination&limit=10&offset=10"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]["directories"]), 0)
        self.assertEqual(len(response.data["results"]["files"]), 10)
        ...
        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir_&pagination&limit=3"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # first page with limit of 3 should return first filtered directories
        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir_1&file_name=0&pagination"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_dirs_only(self):
        ...
        response = self.client.get(
            "/rest/v2/directories/24/files?file_name=_name_11&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 5)
        ...
        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir_5&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["directories"]), 1)
        self.assertEqual(response.data.get("files"), None)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_recursive(self):
        ...
        response = self.client.get("/rest/v2/directories/24/files?file_name=_name_11&recursive")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 10)

        # should have one file from directory with the rest of filtered files
        response = self.client.get(
            "/rest/v2/directories/24/files?directory_name=dir_5&file_name=5&recursive&depth=*"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 6)

    def test_browsing_directory_with_directory_and_file_name_and_cr_identifier_and_not_cr_identifier(
        self,
    ):
        # tests for directory_name and cr_identifier
        response = self.client.get(
            "/rest/v2/directories/17/files?directory_name=2&cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 1)

        response = self.client.get(
            "/rest/v2/directories/17/files?directory_name=phase&cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 2)

        response = self.client.get("/rest/v2/directories/17/files?directory_name=&cr_identifier=13")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 2)

        # tests for directory_name and not_cr_identifier
        response = self.client.get(
            "/rest/v2/directories/17/files?directory_name=phase&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 0)

        response = self.client.get(
            "/rest/v2/directories/17/files?directory_name=2&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["directories"]), 0)

        # tests for file_name and cr_identifier
        response = self.client.get("/rest/v2/directories/12/files?file_name=22&cr_identifier=13")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 1)

        response = self.client.get("/rest/v2/directories/12/files?file_name=name_&cr_identifier=13")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 3)

        response = self.client.get("/rest/v2/directories/12/files?file_name=&cr_identifier=13")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 3)

        response = self.client.get(
            "/rest/v2/directories/17/files?file_name=&cr_identifier=13&directories_only"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(response.data.get("files"), None)
        ...
        response = self.client.get(
            "/rest/v2/directories/16/files?file_name=name&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 1)

        response = self.client.get(
            "/rest/v2/directories/16/files?file_name=name_2&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 1)

        response = self.client.get(
            "/rest/v2/directories/16/files?file_name=name_1&not_cr_identifier=13"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data["files"]), 0)
