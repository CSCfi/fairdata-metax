...
class DirectoryApiReadFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_recursively_with_dirs_only_and_max_depth(self):
        ...
        """
        response = self.client.get('/rest/v2/directories/2/files?recursive=true&directories_only=true&depth=3')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual('directories' in response.data, True)
    ...
    def test_read_directory_recursively_with_no_depth(self):
        ...
        """
        response = self.client.get('/rest/v2/directories/3/files?recursive=true&directories_only=true')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual('directories' in response.data, True)
        self.assertEqual('directories' in response.data['directories'][0], True)
...
class DirectoryApiReadFileBrowsingRetrieveSpecificFieldsTests(DirectoryApiReadCommon):
    ...
    def test_not_retrieving_not_allowed_directory_fields(self):
        ...
        response = self.client.get('/rest/v2/directories/3/files?file_fields=parent,id&directory_fields=;;drop db;,id')
...
class DirectoryApiReadCatalogRecordFileBrowsingTests(DirectoryApiReadCommon):
    ...
    def test_read_directory_for_catalog_record(self):
        ...
        """
        response = self.client.get('/rest/v2/directories/3/files?cr_identifier=%s'
    ...
    def test_directory_byte_size_and_file_count(self):
        ...
        response = self.client.get('/rest/v2/directories/%d/files?cr_identifier=%s' % (dr.id, cr.identifier))
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.get('/rest/v2/directories/%d/files?cr_identifier=%s&include_parent'
            % (dr.id, cr.identifier))
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_directory_byte_size_and_file_count_in_parent_directories(self):
        ...
        def _assertDirectoryData(id, parent_data):
            response = self.client.get('/rest/v2/directories/%d/files?cr_identifier=%d&include_parent' % (id, cr_id))
        ...
        def _assertDirectoryData_not_cr_id(id, parent_data):
            ...
            response = self.client.get('/rest/v2/directories/%s/files?not_cr_identifier=%d&include_parent' %
                (id, cr_id))
        ...
        def _get_parent_dir_data_for_cr(id):
            ...
            grouped_cr_data = {parent_id: [byte_size, file_count] for parent_id, byte_size, file_count in cr_data}
        ...
        dirs = [d['details']['id'] for d in cr.data['research_dataset'].get('directories', [])] + \
            [f['details']['parent_directory']['id'] for f in cr.data['research_dataset'].get('files', [])]
...
class DirectoryApiReadFileNameDirectoryNameTests(DirectoryApiReadCommon):
    ...
    def test_browsing_directory_with_directory_and_file_name(self):
        ...
        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir_1&file_name=file_name_120')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['directories']), 1)
        self.assertEqual(len(response.data['files']), 1)

        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir&file_name=not_existing')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['directories']), 5)
        self.assertEqual(len(response.data['files']), 0)
    ...
    def test_browsing_directory_with_file_and_dir_name_and_pagination(self):
        ...
        response = self.client.get('/rest/v2/directories/24/files?file_name=0&pagination&limit=10&offset=10')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']['directories']), 0)
        self.assertEqual(len(response.data['results']['files']), 10)

        # first page with limit of 3 should return first filtered directories
        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir_&pagination&limit=3')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']['directories']), 3)
        self.assertEqual(len(response.data['results']['files']), 0)

        # first page with limit of 3 should return first filtered directories
        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir_1&file_name=0&pagination')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_dirs_only(self):
        ...
        response = self.client.get('/rest/v2/directories/24/files?file_name=_name_11&directories_only')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data['directories']), 5)
        self.assertEqual(response.data.get('files'), None)

        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir_5&directories_only')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['directories']), 1)
        self.assertEqual(response.data.get('files'), None)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_recursive(self):
        ...
        response = self.client.get('/rest/v2/directories/24/files?directory_name=dir_5&file_name=5&recursive&depth=*')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_browsing_directory_with_directory_and_file_name_and_cr_identifier_and_not_cr_identifier(self):
        ...
        response = self.client.get('/rest/v2/directories/17/files?directory_name=2&cr_identifier=13')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data['directories']), 1)

        response = self.client.get('/rest/v2/directories/17/files?directory_name=phase&cr_identifier=13')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        response = self.client.get('/rest/v2/directories/17/files?file_name=&cr_identifier=13&directories_only')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(response.data.get('files'), None)

        # tests for file_name and not_cr_identifier
        response = self.client.get('/rest/v2/directories/16/files?file_name=name&not_cr_identifier=13')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data['files']), 1)

        response = self.client.get('/rest/v2/directories/16/files?file_name=name_2&not_cr_identifier=13')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data['files']), 1)

        response = self.client.get('/rest/v2/directories/16/files?file_name=name_1&not_cr_identifier=13')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(len(response.data['files']), 0)
