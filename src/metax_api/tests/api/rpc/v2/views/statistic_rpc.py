...
class StatisticRPCCommon(APITestCase, TestClassUtils):
    ...
    def _setup_testdata(self):
        ...
        response = self.client.get('/rest/v2/directories/update_byte_sizes_and_file_counts', format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        response = self.client.get('/rest/v2/datasets/update_cr_total_files_byte_sizes', format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def _create_legacy_dataset(self):
        ...
        response = self.client.post('/rest/v2/datasets', legacy_dataset, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)

        return response.data['id']

    def _create_new_dataset(self, dataset_json):
        response = self.client.post('/rest/v2/datasets', dataset_json, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
    ...
    def _create_new_dataset_version(self, id=1):
        ...
        response = self.client.post(f'/rpc/v2/datasets/create_new_version?identifier={cr["id"]}', format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
...
class StatisticRPCCountDatasets(StatisticRPCCommon, CatalogRecordApiWriteCommon):
    ...
    def test_count_datasets_single(self):
        ...
        response = self.client.get('/rpc/v2/statistics/count_datasets').data
        self.assertEqual(total_count, response['count'], response)

        # test removed -parameter
        self._set_removed_dataset(id=2)
        response = self.client.get('/rpc/v2/statistics/count_datasets?removed=true').data
        self.assertEqual(response['count'], 1, response)

        response = self.client.get('/rpc/v2/statistics/count_datasets?removed=false').data
        self.assertEqual(response['count'], total_count - 1, response)

        # test legacy -parameter
        self._create_legacy_dataset()
        total_count = CatalogRecord.objects_unfiltered.count()
        response = self.client.get('/rpc/v2/statistics/count_datasets?legacy=true').data
        self.assertEqual(response['count'], 1, response)

        response = self.client.get('/rpc/v2/statistics/count_datasets?legacy=false').data
        self.assertEqual(response['count'], total_count - 1, response)
        ...
        self._create_new_dataset_version()
        total_count = CatalogRecord.objects_unfiltered.count()
        response = self.client.get('/rpc/v2/statistics/count_datasets?latest=false').data # returns all
        self.assertEqual(response['count'], total_count, response)

        with_param = self.client.get('/rpc/v2/statistics/count_datasets?latest=true').data
        without_param = self.client.get('/rpc/v2/statistics/count_datasets').data # default is true
    ...
    def test_count_datasets_removed_latest(self):
        ...
        rem_lat = self.client.get('/rpc/v2/statistics/count_datasets?removed=true&latest=true').data
        rem_not_lat = self.client.get('/rpc/v2/statistics/count_datasets?removed=true&latest=false').data
        ...
        not_rem_lat = self.client.get('/rpc/v2/statistics/count_datasets?removed=false&latest=true').data
        not_rem_not_lat = self.client.get('/rpc/v2/statistics/count_datasets?removed=false&latest=false').data
    ...
    def test_count_datasets_removed_legacy(self):
        ...
        rem_leg = self.client.get('/rpc/v2/statistics/count_datasets?removed=true&legacy=true').data
        rem_not_leg = self.client.get('/rpc/v2/statistics/count_datasets?removed=true&legacy=false').data
        not_rem_leg = self.client.get('/rpc/v2/statistics/count_datasets?removed=false&legacy=true').data
        not_rem_not_leg = self.client.get('/rpc/v2/statistics/count_datasets?removed=false&legacy=false').data
    ...
    def test_count_datasets_latest_legacy(self):
        ...
        leg_lat = self.client.get('/rpc/v2/statistics/count_datasets?legacy=true&latest=true').data
        leg_not_lat = self.client.get('/rpc/v2/statistics/count_datasets?legacy=true&latest=false').data
        not_leg_not_lat = self.client.get('/rpc/v2/statistics/count_datasets?legacy=false&latest=false').data
...
class StatisticRPCforDrafts(StatisticRPCCommon, CatalogRecordApiWriteCommon):
    ...
    def test_count_datasets_api_for_drafts(self):
        ...
        """
        response_1 = self.client.get('/rpc/v2/statistics/count_datasets').data
        ...
        response_2 = self.client.get('/rpc/v2/statistics/count_datasets').data
    ...
    def test_all_datasets_cumulative_for_drafts(self):
        """
        Tests that /rpc/v2/statistics/all_datasets_cumulative returns only published datasets
        """
        url = '/rpc/v2/statistics/all_datasets_cumulative?from_date=2019-06&to_date=2019-06'
    ...
    def test_catalog_datasets_cumulative_for_drafts(self):
        """
        Tests that /rpc/v2/statistics/catalog_datasets_cumulative returns only published datasets
        """

        url = '/rpc/v2/statistics/catalog_datasets_cumulative?from_date=2019-06-01&to_date=2019-06-30'
    ...
    def test_end_user_datasets_cumulative_for_drafts(self):
        ...
        url = '/rpc/v2/statistics/end_user_datasets_cumulative?from_date=2019-06-01&to_date=2019-06-30'
    ...
    def test_organization_datasets_cumulative_for_drafts(self):
        ...
        url = "/rpc/v2/statistics/organization_datasets_cumulative?from_date=2019-06-01&to_date=2019-06-30"
