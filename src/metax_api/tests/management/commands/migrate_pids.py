...
class PIDMigrationTest(APITestCase, TestClassUtils):
    ...
    def testMigratePIDFromV3(self):
        ...
        ] = "urn:nbn:fi:att:12345678-abcd-abcd-abcd-12345678912"
    ...
    def testMigratePIDExternalDOI(self):
        ...
        cr["research_dataset"][
            "preferred_identifier"
        ] = "doi:10.12345/12345678-abcd-abcd-abcd-12345678912"
        cr["identifier"] = cr_identifier
        cr["api_meta"] = {"version": 3}
        cr["data_catalog"] = "urn:nbn:fi:att:data-catalog-ida"
        response = self.client.post("/rest/v2/datasets?migration_override", cr, format="json")
    ...
    @responses.activate
    def testMigratePIDAllowFail(self):
        self.mock_responses()
        cr = self.cr
        ...
        catalog_record = CatalogRecord.objects.get(id=cr_id)
        catalog_record_json = catalog_record.research_dataset
        catalog_record_json[
            "preferred_identifier"
        ] = "doi:10.12345/12345678-abcd-abcd-abcd-12345678912"
        catalog_record.research_dataset = catalog_record_json
        catalog_record.force_save()

        self.assertFalse(catalog_record.pid_migrated)

        # Migrate PIDs
        with self.assertLogs() as logs:
            call_command("migrate_pids", "--allow-fail")

        # Assert correct log messages
        msgs = [log.msg % log.args for log in logs.records]
        assert "Succesfully migrated 0 catalog record(s)" in msgs
        assert f"Failed to migrate pids for these datasets: ['{catalog_record.identifier}']" in msgs
        assert (
            f"ValueError('Not inserting PID to PID-MS. CatalogRecord {catalog_record.identifier} PID is doi:10.12345/12345678-abcd-abcd-abcd-12345678912. Only PIDs generated by Metax are inserted to PID-MS')"
            in msgs
        )
