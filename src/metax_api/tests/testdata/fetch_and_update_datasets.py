    ...
    for u in settings.API_USERS:
        if u["username"] == "metax":
            return {
                ...
                % b64encode(bytes("%s:%s" % (u["username"], u["password"]), "utf-8")).decode(
                    "utf-8"
                )
            }
...
def get_test_datasets():
    ...
    response = requests.get(
        "https://localhost/rest/datasets?metadata_owner_org=abc-org-123&pagination=false",
        headers=headers,
        verify=False,
    )
...
def dataset_is_testdata(record):
    ...
    if (
        record["identifier"] != f"cr955e904-e3dd-4d7e-99f1-3fed446f96{end}"
        or record["research_dataset"]["creator"][0]["name"] != "Teppo Testaaja"
    ):
        return False
...
def retrieve_and_update_all_datasets_in_db(headers):
    ...
    response = requests.put(
        "https://localhost/rest/datasets?preserve_version",
        headers=headers,
        data=dumps(test_records),
        verify=False,
    )
...
def retrieve_and_update_all_data_catalogs_in_db(headers):
    ...
    response = requests.get(
        "https://localhost/rest/datacatalogs?limit=100", headers=headers, verify=False
    )
    if response.status_code != 200:
        raise Exception(response.content)
    ...
    print(
        "retrieving details of data catalogs and updating %d data catalogs..."
        % len(data_catalog_ids)
    )

    for dc_id in data_catalog_ids:
        response = requests.get(
            "https://localhost/rest/datacatalogs/%s" % dc_id,
            headers=headers,
            verify=False,
        )
...
def update_directory_byte_sizes_and_file_counts(headers):
    ...
    response = requests.get(
        "https://localhost/rest/directories/update_byte_sizes_and_file_counts",
        headers=headers,
        verify=False,
    )
    if response.status_code not in (200, 201, 204):
...
def update_ida_datasets_total_files_byte_size(headers):
    ...
    response = requests.get(
        "https://localhost/rest/datasets/update_cr_total_files_byte_sizes",
        headers=headers,
        verify=False,
    )
    if response.status_code not in (200, 201, 204):
...
def update_cr_directory_browsing_data(headers):
    ...
    for r in test_records:
        response = requests.get(
            f'https://localhost/rest/datasets/update_cr_directory_browsing_data?id={r["id"]}',
            headers=headers,
            verify=False,
        )
        if response.status_code not in (200, 201, 204):
