...
class ReferenceDataMixin:
    ...
    def check_ref_data(
        ...
        try:
            return next(
                entry for entry in ref_data_type if field_to_check in (entry["uri"], entry["code"])
            )
        except StopIteration:
            if value_not_found_is_error:
                _logger.error("Identifier '%s' not found in reference data" % field_to_check)
                errors[relation_name].append(
                    "Identifier '%s' not found in reference data" % field_to_check
                )
        return None
    ...
    def populate_from_ref_data(
        ref_entry, obj, uri_field="identifier", label_field=None, add_in_scheme=True
    ):
        """
    ...
    @classmethod
    def process_org_obj_against_ref_data(
        cls, orgdata, org_obj, org_obj_relation_name, refdata=None, errors={}
    ):
        """
        ...
        if org_obj.get("identifier", False):
            ref_entry = cls.check_ref_data(
                orgdata,
                org_obj["identifier"],
                org_obj_relation_name + ".identifier",
                value_not_found_is_error=False,
            )
            if ref_entry:
                cls.populate_from_ref_data(
                    ref_entry, org_obj, "identifier", "name", add_in_scheme=False
                )

                if ref_entry.get("parent_org_code", False) and "is_part_of" not in org_obj:
                    parent_ref_entry = cls.check_ref_data(
                        orgdata,
                        ref_entry["parent_org_code"],
        ...
        if refdata and "contributor_type" in refdata:
            for contributor_type in org_obj.get("contributor_type", []):
                ref_entry = cls.check_ref_data(
                    refdata["contributor_type"],
                    contributor_type["identifier"],
                    org_obj_relation_name + ".contributor_type.identifier",
                    errors,
                )
                if ref_entry:
                    cls.populate_from_ref_data(
                        ref_entry, contributor_type, label_field="pref_label"
                    )

    @classmethod
    def process_research_agent_obj_with_type(
        cls, orgdata, refdata, errors, agent_obj, agent_obj_relation_name
    ):
        if agent_obj.get("@type") == "Person":
            ...
            if member_of:
                cls.process_org_obj_against_ref_data(
                    orgdata,
                    member_of,
                    agent_obj_relation_name + ".member_of",
                    refdata=refdata,
                    errors=errors,
                )
            ...
            for contributor_role in agent_obj.get("contributor_role", []):
                ref_entry = cls.check_ref_data(
                    refdata["contributor_role"],
                    contributor_role["identifier"],
                    agent_obj_relation_name + ".contributor_role.identifier",
                    errors=errors,
                )
                if ref_entry:
                    cls.populate_from_ref_data(
                        ref_entry, contributor_role, label_field="pref_label"
                    )
            ...
            for contributor_type in agent_obj.get("contributor_type", []):
                ref_entry = cls.check_ref_data(
                    refdata["contributor_type"],
                    contributor_type["identifier"],
                    agent_obj_relation_name + ".contributor_type.identifier",
                    errors=errors,
                )
                if ref_entry:
                    cls.populate_from_ref_data(
