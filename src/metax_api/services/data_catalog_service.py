    ...
    def validate_reference_data(cls, data_catalog, cache):
        ...
        for language in data_catalog.get("language", []):
            ref_entry = cls.check_ref_data(
                refdata["language"],
                language["identifier"],
                "data_catalog_json.language.identifier",
                errors,
            )
            if ref_entry:
                label_field = "title"
        ...
        for fos in data_catalog.get("field_of_science", []):
            ref_entry = cls.check_ref_data(
                ...
                fos["identifier"],
                "data_catalog_json.field_of_science.identifier",
                errors,
            )
            if ref_entry:
                cls.populate_from_ref_data(
                    ref_entry, fos, label_field="pref_label", add_in_scheme=False
                )
        ...
        if access_rights:
            for access_type in access_rights.get("access_type", []):
                ref_entry = cls.check_ref_data(
                    ...
                    "data_catalog_json.access_rights.access_type.identifier",
                    errors,
                )
                if ref_entry:
                    cls.populate_from_ref_data(
            ...
            for license in access_rights.get("license", []):
                ref_entry = cls.check_ref_data(
                    refdata["license"],
                    license["identifier"],
                    "data_catalog_json.access_rights.license.identifier",
                    errors,
                )
                if ref_entry:
                    cls.populate_from_ref_data(
