...
class FileSerializer(CommonSerializer):
        ...
        if self._operation_is_create:
            ...
            if File.objects.filter(project_identifier=project, file_path=value).exists():
                raise ValidationError(
                    "a file with path %s already exists in project %s" % (value, project)
                )
    ...
    def validate_checksum_algorithm(self, value):
        ...
        if value not in CHECKSUM_ALGORITHMS:
            raise ValidationError(
                "file checksum_algorithm should be one of {}, now {}".format(
                    CHECKSUM_ALGORITHMS, value
                )
            )
    ...
    def _get_file_storage_relation(self, identifier_value):
        ...
        try:
            ...
            raise ValidationError(
                {"file_storage": ["identifier %s not found" % str(identifier_value)]}
            )
    ...
    def _get_parent_directory_relation(self, identifier_value):
        ...
        try:
            ...
            raise ValidationError(
                {"parent_directory": ["identifier %s not found" % str(identifier_value)]}
            )
