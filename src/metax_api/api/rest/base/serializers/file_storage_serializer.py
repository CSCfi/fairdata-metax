...
class FileStorageSerializer(CommonSerializer):
    ...
    def _validate_identifier_uniqueness(self, file_storage_json):
        if FileStorage.objects.filter(file_storage_json__identifier=file_storage_json['identifier']).exists():
            raise ValidationError({'identifier':
                ['identifier %s already exists' % file_storage_json['identifier']]
            })
