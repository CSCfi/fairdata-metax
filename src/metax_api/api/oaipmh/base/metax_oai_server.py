...
class MetaxOAIServer(ResumptionOAIPMH):
    ...
    def identify(self):
        first = CatalogRecord.objects.filter(data_catalog_id__in=self._get_set_filter()).order_by('date_created').values_list('date_created', flat=True).first()
        if first:
    ...
    def getRecord(self, metadataPrefix, identifier):
        record = CatalogRecord.objects.filter(**{ 'research_dataset__contains': {'urn_identifier': identifier } }).first()
        if not record:
