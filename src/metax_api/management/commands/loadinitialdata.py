    ...
    def _load_data_catalogs(self):
        ...
        for dc in data_catalogs:
            response = requests.post(
                "%s/rest/v2/datacatalogs" % self._metax_api_root,
                json=dc,
            ...
            if response.status_code == 201:
                self.stdout.write(
                    "Created catalog: %s" % dc["catalog_json"]["identifier"]
                )
            else:
                # update instead
                try:
                ...
                if self._error_is_already_exists(errors.get("catalog_json", {})):
                    self.stdout.write(
                        "Catalog %s already exists, updating instead..."
                        % dc["catalog_json"]["identifier"]
                    )

                    response = requests.put(
                        "%s/rest/v2/datacatalogs/%s"
                ...
                raise CommandError(
                    "Failed to process catalog: %s. Reason: %s"
